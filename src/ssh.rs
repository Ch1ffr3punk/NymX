...
pub fn parse_host(host_spec: &str) -> (String, String) {
    if let Some(at_pos) = host_spec.find('@') {
        let username = host_spec[..at_pos].to_string();
        let host = host_spec[at_pos + 1..].to_string();
        (host, username)
    } else {
        (host_spec.to_string(), String::new())
    }
}
