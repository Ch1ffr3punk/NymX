...
pub async fn run_get_mode() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let ssh_config = config.ssh.ok_or("No SSH config in nymx.json")?;
    let host = &ssh_config.host;
    let port = ssh_config.port;
    let proxy = &ssh_config.socks5_proxy;
    let username = &ssh_config.username;
    let remote_path = &ssh_config.remote_path;

    if host.is_empty() || username.is_empty() {
        return Err("Username and host are required in nymx.json".into());
    }

    println!("Checking Tor SOCKS5 proxy at {}...", proxy);
    match TcpStream::connect(proxy) {
        Ok(_) => println!("Tor proxy is reachable"),
        Err(e) => {
            eprintln!("Cannot connect to Tor proxy at {}: {}", proxy, e);
            eprintln!("Please start Tor: systemctl start tor (Linux) or run tor.exe (Windows)");
            return Err("Tor proxy not available".into());
        }
    }
    ...
    let session = connect_ssh(host, port, username, &password, Some(proxy))?;
    drop(password);

    let remote_dir = format!("/home/{}/{}", username, remote_path);
