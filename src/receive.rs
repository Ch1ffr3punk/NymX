...
pub async fn receive_mode(
    ...
    loop {
        println!("[INFO] Connecting to Nym Mixnet...");
            ...
            .await
        {
            Ok(builder) => match builder.build() {
                Ok(built) => match built.connect_to_mixnet().await {
                    ...
                    Err(e) => {
                        eprintln!("[ERROR] Connection failed: {}. Retrying in {}s...", e, RECONNECT_DELAY_SECS);
                        tokio::time::sleep(Duration::from_secs(RECONNECT_DELAY_SECS)).await;
                        continue;
                    }
                },
                Err(e) => {
                    eprintln!("[ERROR] Build failed: {}. Retrying in {}s...", e, RECONNECT_DELAY_SECS);
                    tokio::time::sleep(Duration::from_secs(RECONNECT_DELAY_SECS)).await;
                    continue;
                }
            },
            Err(e) => {
                eprintln!("[ERROR] Builder failed: {}. Retrying in {}s...", e, RECONNECT_DELAY_SECS);
                tokio::time::sleep(Duration::from_secs(RECONNECT_DELAY_SECS)).await;
                continue;
            }
        };
            ...
            tokio::select! {
                ...
                messages_result = client.wait_for_messages() => {
                    let messages = match messages_result {
                        Some(msgs) => msgs,
                        None => {
                            eprintln!("[ERROR] Connection to Mixnet lost (received None)");
                            break true;
                        }
                    };
        ...
        println!("[WARN] Connection lost. Reconnecting in {} seconds...", RECONNECT_DELAY_SECS);
        tokio::time::sleep(Duration::from_secs(RECONNECT_DELAY_SECS)).await;
    }
