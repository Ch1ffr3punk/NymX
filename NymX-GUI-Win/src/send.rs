...
pub fn set_log_sender(tx: mpsc::Sender<String>) {
...
macro_rules! log_err {
    ($($arg:tt)*) => {
        if let Some(tx) = LOG_SENDER.get() {
            let _ = tx.send(format!("ERROR: {}", format!($($arg)*)));
        } else {
            eprintln!($($arg)*);
        }
    };
...
pub async fn send_mode(address: String, file_path: PathBuf) -> Result<(), Box<dyn std::error::Error>> {
    ...
    log!("Your ephemeral Nym address: {}", our_address);
    log!("Sending anonymously to: {}\n", address);
    ...
    log!("File size: {} MiB", file_size_mib);
    log!("Original filename: {}", filename);
    log!("Splitting into {} chunks of {} KB each", total_chunks, CHUNK_SIZE / 1024);

    let sender_hash = ripemd160_file(&file_path)?;
    log!("Sender hash: {}", sender_hash);
    ...
    let target_address: Recipient = match address.parse() {
        Ok(addr) => addr,
        Err(e) => {
            log_err!("Error: Invalid Nym address format");
            log_err!("Details: {}", e);
            log_err!("\nA valid Nym address has the format:");
            log_err!("  <Base58>.<Base58>@<Base58>");
    ...
    if let Err(e) = client.send_plain_message(target_address, handshake_payload).await {
        log_err!("Failed to send handshake: {}", e);
        client.disconnect().await;
        let elapsed = start_time.elapsed();
        log!("Done!");
        log!("Total time: {}", format_duration(elapsed));
        return Ok(());
    }

    log!("Waiting up to {}s for recipient response...", HANDSHAKE_TIMEOUT_SECS);
    ...
    let handshake_ok = match handshake_result {
        Ok(reply) => {
            ...
            if reply_str.starts_with(&expected_prefix) {
                log!("Recipient is online and ready!\n");
                true
            } else {
                log!("Unexpected response from recipient");
                false
            }
        }
        Err(_) => {
            log!("No response within {}s", HANDSHAKE_TIMEOUT_SECS);
            false
        }
    };
    ...
    if !failed_chunks.is_empty() {
        ...
        for chunk_idx in &failed_chunks {
            log!("  - Chunk {}", chunk_idx + 1);
        }
        log!("The receiver may have an incomplete file.");
    }
                ...
                if msg_str.starts_with(RESEND_REQUEST_PREFIX) {
                    if let Some(chunk_idx_str) = msg_str.strip_prefix(RESEND_REQUEST_PREFIX) {
                        if let Ok(chunk_idx) = chunk_idx_str.parse::<usize>() {
                            log!("Resend request received for chunk {}", chunk_idx + 1);
                            ...
                            match seek_result {
                                Ok((buffer, bytes_read)) => {
                                    ...
                                    if let Err(e) = client.send_plain_message(target_address_clone, chunk_payload).await {
                                        log_err!("Failed to resend chunk {}: {}", chunk_idx + 1, e);
                                    } else {
                                        log!("  Chunk {} resent", chunk_idx + 1);
                                    }
                                }
                                Err(e) => {
                                    log_err!("Failed to read chunk {} from file: {}", chunk_idx + 1, e);
                                }
                            }
                        }
                    }
    ...
    let reply_message = match reply_message {
        Some(msg) => msg,
        None => {
            log!("Timeout: No reply received. The receiver likely has an incomplete file.");
            client.disconnect().await;
            let elapsed = start_time.elapsed();
            log!("Done!");
            log!("Total time: {}", format_duration(elapsed));
...
pub async fn send_parts_mode(address: String, prefix: PathBuf) -> Result<(), Box<dyn std::error::Error>> {
    ...
    log!("=== Parts Mode ===");
    log!("Looking for files with prefix: {}", prefix_str);
    ...
    log!("Found {} expected parts in ripemd-160.txt\n", expected_parts.len());
    ...
    for (i, (filename, expected_hash, file_path_str)) in part_files.iter().enumerate() {
        ...
        log!("\n========================================");
        log!("Sending part {}/{}: {}", i + 1, part_files.len(), filename);
        log!("========================================\n");
        ...
        match send_single_part(&mut client, target_address, &file_path, expected_hash).await {
            Ok(true) => {
                ...
                log!("  Part time: {}", format_duration(part_elapsed));
        ...
        if i < part_files.len() - 1 {
            log!("Waiting 2 seconds before next part...");
            sleep(Duration::from_secs(2)).await;
        }
    }

    log!("\n========================================");
    log!("=== Parts Transfer Summary ===");
    log!("========================================");
    log!("Total parts: {}", part_files.len());
    log!("Successful: {}", successful_parts);
    log!("Failed: {}", failed_parts.len());

    if !failed_parts.is_empty() {
        log!("\nFailed parts:");
        for (num, filename, error) in &failed_parts {
            log!("  Part {}: {} - {}", num, filename, error);
        }
    }
...
fn parse_ripemd_file() -> Result<HashMap<String, String>, Box<dyn std::error::Error>> {
    ...
    for line in content.lines() {
        ...
        if line.starts_with("RIPEMD") || line.starts_with("Operation") ||
        ...
        if let Some(colon_pos) = line.rfind(':') {
            ...
            if let Some(paren_pos) = before_colon.rfind('(') {
                ...
                if let Some(space_pos) = before_colon.rfind(' ') {
                    let filename = before_colon[..space_pos].trim().to_string();
                    if !filename.is_empty() && !hash.is_empty() {
                        parts.insert(filename, hash);
                    }
                }
