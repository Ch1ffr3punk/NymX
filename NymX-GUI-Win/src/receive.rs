...
pub async fn receive_mode(
    ...
    loop {
        log!("[INFO] Connecting to Nym Mixnet...");
            ...
            .await
        {
            Ok(builder) => match builder.build() {
                Ok(built) => match built.connect_to_mixnet().await {
                    ...
                    Err(e) => {
                        log_err!("[ERROR] Connection failed: {}. Retrying in {}s...", e, RECONNECT_DELAY_SECS);
                        tokio::time::sleep(Duration::from_secs(RECONNECT_DELAY_SECS)).await;
                        continue;
                    }
                },
                Err(e) => {
                    log_err!("[ERROR] Build failed: {}. Retrying in {}s...", e, RECONNECT_DELAY_SECS);
                    tokio::time::sleep(Duration::from_secs(RECONNECT_DELAY_SECS)).await;
                    continue;
                }
            },
            Err(e) => {
                log_err!("[ERROR] Builder failed: {}. Retrying in {}s...", e, RECONNECT_DELAY_SECS);
                tokio::time::sleep(Duration::from_secs(RECONNECT_DELAY_SECS)).await;
                continue;
            }
        };

        log!("Your Nym address: {}", client.nym_address());
        log!("Saving files to: {}", receive_path.display());
        log!("Transfer timeout: {} seconds", TRANSFER_TIMEOUT_SECS);
        log!("Direct append mode");
        log!("Waiting for anonymous incoming files...\n");
        ...
        let connection_lost = loop {
            tokio::select! {
                _ = signal::ctrl_c() => {
                    ...
                    if active_count == 0 {
                        log!("[INFO] No active transfers. Shutting down cleanly.");
                    } else {
                        log!("[WARN] {} active transfer(s) still in progress.", active_count);
                        log!("[INFO] Because chunks are sync'd to disk immediately, data is safe.");
                        log!("[INFO] Finalizing client disconnect. Partial files can be resumed later.");
                    }
                    break false;
                }
                ...
                messages_result = client.wait_for_messages() => {
                    let messages = match messages_result {
                        Some(msgs) => msgs,
                        None => {
                            log_err!("[ERROR] Connection to Mixnet lost (received None)");
                            break true;
                        }
                    };
                    ...
                    for message in messages {
                        ...
                        if chunk_idx == HANDSHAKE_CHUNK_IDX as usize {
                            ...
                            if let Some(sender_tag) = &message.sender_tag {
                                ...
                                log!("  Replied with READY");
                            } else {
                                log!("  No sender tag in handshake, cannot reply");
                            }
                            continue;
                        }
                        ...
                        let transfer = transfers_lock
                            ...
                            .or_insert_with(|| {
                                match FileTransfer::new(filename.clone(), app_tag.clone(), total_chunks, file_size, &receive_path) {
                                    Ok(t) => {
                                        log!("New transfer started for '{}'", filename);
                                        log!("Chunks: {}", total_chunks);
                                        t
                                    }
                                    Err(e) => {
                                        log_err!("Failed to create target file: {}", e);
                        ...
                        if transfer.is_complete() {
                            ...
                            if let Err(e) = set_file_times(&final_path, epoch, epoch) {
                                log_err!("[ERROR] Failed to set file times: {}", e);
                            }
                            ...
                            log!("File saved to: {}", final_path.display());
                            log!("File size: {} MiB", file_size / 1024 / 1024);
                                ...
                                if let Some(sender_tag) = sender_tag_clone {
                                    for chunk_idx in missing {
                                        if transfer.can_request_resend(chunk_idx) {
                                            ...
                                            log!("  Requested resend for chunk {}", chunk_idx + 1);
                                        } else {
                                            log!("  Max resend attempts reached for chunk {}", chunk_idx + 1);
                                        }
                                    }
                                }
                            }
        ...
        log!("[INFO] Disconnecting from Nym Mixnet...");
        ...
        log!("[WARN] Connection lost. Reconnecting in {} seconds...", RECONNECT_DELAY_SECS);
        tokio::time::sleep(Duration::from_secs(RECONNECT_DELAY_SECS)).await;
    }
